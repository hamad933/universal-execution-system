...
def test_exact_fingerprint_and_repo_survive_provider_branch_drift():
    ...
    assert result["provider_starting_branch_drift"] is True
...
def test_matching_branch_reports_no_drift():
    result = match_lineage_session(
        [_session(fp="a" * 64, branch="main")],
        {
            "known_session_fingerprints": ["a" * 64],
    ...
    assert result["provider_starting_branch_drift"] is False
