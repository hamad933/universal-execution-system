...
class LineageRegistryTests(unittest.TestCase):
    ...
    def test_exact_fingerprint_uses_repository_without_conflating_pr_head_branch(self) -> None:
        ...
        sessions = [{
            "name": "sessions/exact", "normalizedState": "COMPLETED", "_source_repository": "owner/repo",
            "_session_fingerprint": expected, "sourceStartingBranch": "provider-base",
        }]
        result = match_lineage_session(
            sessions,
            {"known_session_fingerprints": [expected], "pr_head_branch": "work/output"},
    ...
    def test_optional_provider_starting_branch_is_exact_additional_constraint(self) -> None:
        ...
        rejected = match_lineage_session(
            sessions,
            {"known_session_fingerprints": [expected], "provider_starting_branch": "other-base"},
            repository="owner/repo",
        )
    ...
    def test_labels_never_substitute_for_exact_binding(self) -> None:
        sessions = [{
            "name": "sessions/wrong", "normalizedState": "IN_PROGRESS", "_source_repository": "owner/repo",
            "_session_fingerprint": session_fingerprint("sessions/wrong"), "sourceStartingBranch": "wrong-branch", "title": "W03 Writer",
        }]
        result = match_lineage_session(
            sessions,
            {"known_session_fingerprints": [], "provider_starting_branch": "work/w03"},
            repository="owner/repo",
        )
    ...
    def test_generation_changes_only_when_bound_provider_session_changes(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            binding1 = {
                ...
                "session": {"_source_repository": "owner/repo", "sourceStartingBranch": "provider/w01"},
            }
...
class RecoveryCatalogTests(unittest.TestCase):
    ...
    def test_pr_branch_drift_precedes_provider_action(self) -> None:
        result = plan_recovery({
            "pr_branch_match": False, "binding_status": "PROVEN", "provider_state": "AWAITING_USER_FEEDBACK",
            "role": "WRITER", "same_session_prompt_ready": True,
        })
    ...
    def test_completed_reviewer_findings_route_to_writer(self) -> None:
        result = plan_recovery({
            ...
            "current_sha": "b" * 40, "handoff": {"status": "COMPLETE", "verdict": "FINDINGS", "reviewed_sha": "b" * 40},
            "work_remaining": True,
        })
