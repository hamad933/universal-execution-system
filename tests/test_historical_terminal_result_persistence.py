...
from ues.terminal_recovery import (
    HISTORICAL_TERMINAL_RESULTS_KEY,
    TERMINAL_RESULT_KEY,
...
HISTORICAL_KEY = HISTORICAL_TERMINAL_RESULTS_KEY
CURRENT_FP = "c" * 64
HISTORICAL_FP = "b" * 64
UNPROVEN_FP = "d" * 64
...
class FakeStore:
    ...
    def compare_and_swap_workstream(self, lane_id, expected_version, record):
        self.cas_calls += 1
        if self.move_before_cas:
            self.move_before_cas = False
            moved = WorkstreamRuntimeRecord.from_dict(self.record.to_dict())
            bindings = dict(moved.evidence_bindings or {})
            bindings["generation"] = 4
            bindings["session_fingerprint"] = "e" * 64
            bindings["previous_session_fingerprint"] = CURRENT_FP
            moved.evidence_bindings = bindings
            self.version += 1
            self.record = moved
            raise StateVersionConflict("simulated concurrent lineage movement")
...
class HistoricalTerminalResultPersistenceTests(unittest.TestCase):
    def test_exact_historical_result_persists_without_rewinding_current_lineage(self):
        ...
        self.assertEqual(entry["persistence_scope"], "HISTORICAL_EXACT_BOUND")
    ...
    def test_conflicting_historical_result_fails_closed_without_second_cas(self):
        lane_id, store = _store()
        first = persist_terminal_result(
            store,
            result=_historical_result(),
        ...
        conflicting = _historical_result()
        conflicting["result_fingerprint"] = "conflicting-result-2"
        ...
        self.assertEqual(first["state"], "HISTORICAL_TERMINAL_RESULT_PERSISTED")
        self.assertEqual(second["state"], "HISTORICAL_TERMINAL_RESULT_CONFLICT")
        self.assertEqual(store.cas_calls, 1)
        self.assertFalse(second["safe_to_blind_retry"])

    def test_unproven_historical_identity_fails_closed(self):
        lane_id, store = _store()
        result = _historical_result()
        result["session_fingerprint"] = UNPROVEN_FP
        lineage = _historical_lineage(lane_id)
        outcome = persist_terminal_result(store, result=result, lineage=lineage)
        self.assertEqual(outcome["state"], "HISTORICAL_TERMINAL_RESULT_IDENTITY_NOT_EXACT")
        self.assertEqual(store.cas_calls, 0)
        self.assertNotIn(HISTORICAL_KEY, store.record.evidence_bindings)

    def test_concurrent_current_lineage_movement_fails_closed_without_rewind(self):
        lane_id, store = _store()
        store.move_before_cas = True
        outcome = persist_terminal_result(
            store,
            result=_historical_result(),
            lineage=_historical_lineage(lane_id),
        )
        self.assertEqual(outcome["state"], "HISTORICAL_TERMINAL_RESULT_CAS_CONFLICT")
        self.assertEqual(store.record.evidence_bindings["generation"], 4)
        self.assertEqual(store.record.evidence_bindings["session_fingerprint"], "e" * 64)
        self.assertNotIn(HISTORICAL_KEY, store.record.evidence_bindings)
        self.assertFalse(outcome["safe_to_blind_retry"])

    def test_exact_historical_result_is_readable_as_parent_consumable(self):
        lane_id, store = _store()
        persist_terminal_result(
            store,
            result=_historical_result(),
        ...
        results = read_persisted_terminal_results(
            store,
            project="RP02",
            route="RP02",
            repository="hamad933/Enterprise-Operations-Control",
        )
        historical = [item for item in results if item.get("generation") == 2]
        self.assertEqual(len(historical), 1)
        self.assertEqual(historical[0]["result_state"], "PARENT_CONSUMABLE")
        self.assertEqual(historical[0]["persistence_scope"], "HISTORICAL_EXACT_BOUND")
