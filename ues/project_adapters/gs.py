...
# Current governed GS authority does not grant any autonomous external effect.
# A future authority source may supply a bounded allowlist at runtime, but absence
# of such a source must remain fail-closed.
...
def build_evidence_profile(
    *,
    exact_head_ci_proven: bool,
    exact_sha_review_proven: bool,
    reviewer_contract_clean: bool,
    browser_profile_proven: bool | None = None,
    exact_head_ci_evidence_id: str | None = None,
    exact_sha_review_evidence_id: str | None = None,
    reviewer_contract_evidence_id: str | None = None,
    ...
    requirements = [
        EvidenceRequirement(
            "gs_exact_head_required_ci",
            exact_head_ci_proven,
            evidence_id=exact_head_ci_evidence_id,
        ...
        EvidenceRequirement(
            "gs_reviewer_contract_clean",
            reviewer_contract_clean,
            evidence_id=reviewer_contract_evidence_id,
            actions=parent_actions,
        ),
    ]
    if browser_profile_proven is not None:
        requirements.append(
            EvidenceRequirement(
                "gs_required_browser_profile",
                browser_profile_proven,
                evidence_id=browser_evidence_id,
                actions=parent_actions,
            )
        )
    return RequiredEvidenceProfile("gs-shadow-evidence-v1", tuple(requirements))
...
def validate_binding(binding: WorkstreamBinding) -> None:
    ...
    if observed != expected:
        raise ValueError(
            "GS adapter identity mismatch: expected project/route/repository "
