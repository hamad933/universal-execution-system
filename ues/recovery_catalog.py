...
def plan_recovery(observation: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if binding == "AMBIGUOUS":
        return _decision("RECONCILE_EXACT_LINEAGE_BINDING", root_cause="MULTIPLE_EXACT_LINEAGE_SESSION_MATCHES", executable=True)
    if binding == "UNBOUND":
    ...
    if current_sha and reviewed_sha and current_sha.lower() != reviewed_sha.lower():
        return _decision("INVALIDATE_STALE_REVIEW_AND_ROUTE_CURRENT_SHA", root_cause="REVIEW_SHA_STALE", executable=True)

    if state == "AWAITING_USER_FEEDBACK":
        if waiting_has_newer_user is True:
            return _decision("WAIT_FOR_AGENT_ON_SAME_SESSION", root_cause="WAITING_ALREADY_HAS_NEWER_OR_EQUAL_USER_RESPONSE", executable=True)
        if observation.get("same_session_prompt_ready"):
            return _decision("CONTINUE_SAME_SESSION", root_cause="WAITING_INPUT_CONTROLLER_RESOLVABLE", executable=True, external_effect=True)
        return _decision("RESOLVE_WAITING_INPUT_CONTENT_OR_POLICY", root_cause="WAITING_INPUT_REQUIRES_RECONCILIATION", executable=True)
    ...
    if state == "AWAITING_PLAN_APPROVAL":
        return _decision("APPROVE_PLAN_OR_PARENT_RECONCILE_SAME_SESSION", root_cause="PLAN_APPROVAL_REQUIRED", executable=True)
    if state == "PAUSED":
    ...
    if state == "COMPLETED":
        if role == "WRITER":
            ...
            if candidate_sha:
                if ci_reason == "REQUIRED_CI_MISSING":
                    return _decision("DIAGNOSE_AND_REMEDIATE_CI_TRIGGER", root_cause="CI_TRIGGER_OR_EVIDENCE_MISSING", executable=True)
                if str(observation.get("ci_verdict") or "").upper() == "PASS":
                    return _decision("ROUTE_CURRENT_SHA_TO_REVIEWER_LINEAGE", root_cause="WRITER_COMPLETED_CANDIDATE_READY_FOR_REVIEW", executable=True)
                return _decision("VALIDATE_EXACT_WRITER_CANDIDATE", root_cause="WRITER_COMPLETED_REQUIRES_EXACT_HEAD_EVIDENCE", executable=True)
            if work_remaining:
        ...
        if role in {"REVIEWER", "ASSURANCE"}:
            if verdict == "PASS" and reviewed_sha and (not current_sha or reviewed_sha.lower() == current_sha.lower()):
                return _decision("MARK_CURRENT_SHA_REVIEW_PASS", root_cause="STRUCTURED_REVIEW_PASS_CURRENT_SHA", executable=True)
            if verdict in {"FINDINGS", "FAIL"}:
                return _decision("ROUTE_STRUCTURED_FINDINGS_TO_WRITER_LINEAGE", root_cause="REVIEW_FINDINGS_REQUIRE_CORRECTION", executable=True, external_effect=True)
            if work_remaining:
                return _replacement_decision(budget_safe, replacement_prompt_ready, "REVIEWER_COMPLETED_MORE_REVIEW_REQUIRED")
            return _decision("PARENT_ADJUDICATE_UNSTRUCTURED_COMPLETED_REVIEW", root_cause="COMPLETED_REVIEW_OUTPUT_UNSTRUCTURED_OR_UNBOUND", executable=True)

    if state == "FAILED":
        if not work_remaining:
            return _decision("CLOSE_FAILED_LINEAGE_NO_REPLACEMENT", root_cause="TERMINAL_FAILURE_NO_MORE_WORK_REQUIRED", executable=True)
    ...
    if observation.get("unknown_write_state"):
        return _decision("AUTHORITATIVE_POST_WRITE_RECONCILIATION", root_cause="UNKNOWN_PROVIDER_WRITE", executable=True)

    return _decision("ROOT_CAUSE_RECONCILIATION_REQUIRED", root_cause="UNCLASSIFIED_LIFECYCLE_STATE", executable=True)


def _replacement_decision(budget_safe: bool, prompt_ready: bool, reason: str) -> dict[str, Any]:
    if budget_safe and prompt_ready:
        return _decision("CREATE_NEXT_SESSION_GENERATION_SAME_LINEAGE", root_cause=reason, executable=True, external_effect=True)
    missing = []
    if not budget_safe:
...
def _decision(action: str, *, root_cause: str, executable: bool, external_effect: bool = False, stop_gate: str | None = None) -> dict[str, Any]:
    return {
